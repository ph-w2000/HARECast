...
class Meteo(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.data_path,'r') as f:
            ...
            ir108_imgs = f[self.type][str(index)+"_ir108"][()]  # numpy array: (565, 784), dtype=uint8, range(0,70)

            frames = torch.from_numpy(imgs).float().squeeze() 
            frames = frames / self.pixel_scale
            frames = self.transform(frames)     

            ir108_imgs = np.repeat(ir108_imgs[None, :, :], 5, axis=0)
            ir108_frames = torch.from_numpy(ir108_imgs).float()
            ir108_frames = ir108_frames / self.pixel_scale
            ir108_frames = self.transform(ir108_frames)  
        return frames.unsqueeze(1), ir108_frames.unsqueeze(1) # (25,1,128,128)
