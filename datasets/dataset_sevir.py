            ...
            if key in data_types:
                if isinstance(data, np.ndarray):
                    ...
                    if key == 'ir107':
                        min_val = -7000
                        max_val = 2000
                        data = (data.float() - min_val) / (max_val - min_val)
...
class SEVIRTorchDataset(TorchDataset):
    ...
    def __getitem__(self, index):
        ...
        data_vil = data_dict["vil"]
        data_ir107 = data_dict["ir107"]
        data_vil = self.transform(data_vil).unsqueeze(2)
        data_ir107 = self.transform(data_ir107).unsqueeze(2)

        return data_vil, data_ir107
